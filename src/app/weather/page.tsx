...
// Interfaces for typed API responses
...
interface SixteenDayForecastResponse {
  list: {
    dt: number;
    temp: { day: number };
    weather: { icon: string; main: string }[];
  ...
  const loadWeather = useCallback(async () => {
    try {
      ...
      const weatherPromises = locationsData.map(async (loc: Location) => {
        ...
        let fiveDayRes: FiveDayForecastResponse | null = null;
        let sixteenDayRes: SixteenDayForecastResponse | null = null;
        ...
        const historical: ForecastItem[] = Array.from({ length: 3 }).map((__, idx) => {
          const date = new Date();
...
<div className="mt-4 pt-4 border-t border-gray-200 border-opacity-50">
  ...
  <div className="grid grid-cols-5 gap-2">
    {weather.forecast5.map((day) => (
      <div
        key={day.date}
        className="flex flex-col items-center bg-white bg-opacity-50 rounded p-2"
      >
